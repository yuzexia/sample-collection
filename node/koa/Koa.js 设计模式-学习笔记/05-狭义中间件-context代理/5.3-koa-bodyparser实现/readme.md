...
- 除了在实例化`let app = new koa();`的时候将属性或方法挂载到`app.context`中，供后续中间件使用，
- 另外一种方式是在请求过程中在顶端中间件（一般在第一个中间件）使用，把数据或方法挂载代理到ctx供下游中间件获取和使用

> 常见请求代理上下文context实现过程

- 请求代理ctx
- 直接app.use()
- 在请求过程中挂载方法或数据到上下文ctx
...
- 02，拦截post请求
- 03，等待解析表单信息
- 04，把表单信息代理到ctx.request.body上
- 05，下游中间件都可以在ctx.request.body中获取表单数据
